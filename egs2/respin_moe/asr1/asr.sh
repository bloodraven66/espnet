...
if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ] && ! [[ " ${skip_stages} " =~ [[:space:]]11[[:space:]] ]]; then
    ...
    if ${use_dialectid}; then
        _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/utt2dialectid,dialectid,text_int "
        _opts+="--valid_data_path_and_name_and_type ${_asr_valid_dir}/utt2dialectid,dialectid,text_int "
    fi
...
if [ ${stage} -le 12 ] && [ ${stop_stage} -ge 12 ] && ! [[ " ${skip_stages} " =~ [[:space:]]12[[:space:]] ]]; then
    ...
    for dset in ${_dsets}; do
        ...
        if ${use_dialectid}; then
        _opts+="--data_path_and_name_and_type ${_data}/utt2dialectid,dialectid,text_int "
        fi
